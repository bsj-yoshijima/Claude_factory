...
☰メニューの🏆リーダーボードで [スコアカード](docs/wp.md#9-業務スコアカード) を軸ごとに並べ替えできる。

**リーダーボードに並ぶのは「自分と同じグループに居る人」だけ。** アカウントを作ると
既定グループ（`DEFAULT_GROUP`、既定値 `bravesoft`）へ自動で入るので、社内で使っている
うちは全員が同じ一覧に並ぶ。どのグループにも入っていない人には ☰メニューに項目自体が
出ない（API も 403 を返す）。グループの作成・招待は社外展開のときに別途作る。設計は
[docs/multiuser.md](docs/multiuser.md#7-リーダーボードはグループ単位で見せる)。
...
`node test/test_craft.mjs` がこのブロックを切り出して、キーの正規化・ジャンルの整合・確率（実測が設定値に寄るか）・全製品の到達性・**製品と組み合わせが1対1であること**・原材料数が2〜5に収まることまで検証する。**`m` を触ったら必ず走らせる。**
...
> 計測ロジックの全体・較正データ・**指標としての妥当性の限界**は [docs/wp.md](docs/wp.md) にまとめてある。
>
...
> 使い方を見るための多軸スコアカードは [docs/wp.md #9](docs/wp.md#9-業務スコアカード)。
...
| `docs/prop-sprite-prompt.md` / `docs/prop-prompts/` | 装飾品の規格と失敗の記録 ／ 31テーマ×5種の発注プロンプト（生成済み。そのまま貼れる） |
...
| やりたいこと | やること |
|---|---|
| 新しいテーマのドット絵を足す | `assets/mach-sheets/<theme>.png`（2x2・マゼンタ背景・左上から2/3/4/5マス）を置いて `python3 tools/assets/cut_machines.py`。`MACH_ART` に `<theme>` を足す |
