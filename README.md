...
1. 🔧編集で **抽出機（🟥）を最大3台**設置する
2. 抽出機をクリック → **原材料を1つ選ぶ**（牛乳・小麦粉・卵・バターなど12種）
3. 1台以上にセットすると左下の製造パネルに **▶製造開始** が出る
4. 押した時点からの **実WP** がカウントされ、**50WP で1製品が完成**
5. 完成すると右上に **🎁完成品** ボタン。開くと一覧が出て**📖図鑑に登録**される（初回は `NEW`）

| 仕様 | 挙動 |
|---|---|
| 製品の決まり方 | 原材料の組み合わせごとに**出うる製品が5種**あり、その中からレア度で重み付けしてランダム |
| 未定義の組み合わせ | 原材料との相性でスコアリングして上位5種を選ぶ。**組み合わせが同じなら常に同じ5種**（ハッシュ由来なので開き直しても変わらない） |
| 超過WP | 次の製品に繰り越す |
| 原材料の変更 | セッション途中でも**WPは0にリセット**される |
| セッションの永続 | リロードしても継続（閉じている間に稼いだWPも反映される） |

工場ビュー左下に**現セッションWP / 今日の累計WP**を表示。🏆リーダーボードタブで [スコアカード](WP.md#9-業務スコアカード) を軸ごとに並べ替えできる。
