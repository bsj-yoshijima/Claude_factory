...
| 増やすもの | 足す場所 |
|---|---|
| ジャンル | `GENRES` に1行。図鑑のタブ・原材料ピッカーのタブは自動で増える |
| 原材料 | `MATS` に1行（`g` = ジャンル） |
| 製品 | `PRODS` に1行（`g` = ジャンル / `r` = レア度1..5 / `m` = 図鑑ヒント用の原材料） |
| 組み合わせ | `RECIPES` にキー1行（キー = 原材料idを重複除去してソートしカンマ結合） |
| ジャンル跨ぎの隠し当たり | `SECRETS` に1行（`p` = シークレットが出る確率。外れは 🪨） |

**確率の書き方は3通り**。用途で選ぶ（`normPool()` が正規化する）。

```js
'glass,wire'          : ['bulb','light','tv','camera','clock'],          // レア度の既定重み(RAR[r].w)で抽選
'glass,iron,semic,wire': {pc:55, printer:25, tv:12, robot:6, satellite:2}, // 相対確率を明示（合計は任意）
'alum,iron,magnet,wire': [['train',52],['printer',20],['robot',14]],       // 同上。並び順を保ちたいとき
```

抽選の分岐は `rollProduct()` の4行だけ: ①隠しレシピ（跨ぎ）→ 確率でシークレット / 外れは🪨 → ②跨ぎでレシピ無し → 🪨 → ③同ジャンルのレシピ → 重み付き抽選 → ④レシピ無し → 🪨。

`node tools/test_craft.mjs` がこのブロックを切り出して、キーの正規化・ジャンルの整合・確率（実測が設定値に寄るか）・全製品の到達性まで検証する。**レシピを足したら必ず走らせる。**
