...
> **この段階は完了している。** 現在はアプリも Cloud Run で動いているので、
> 各自の PC でサーバを立てる必要はない（→ [deploy.md](deploy.md)）。
>
> この文書は経緯として残してある。中でいまも有効なのは次の3つ。
>
> - **Neon のプロジェクトの作り方**（リージョン / pooler を使わない理由 / タイムゾーン）
> - **Google OAuth クライアントの取得手順**と、`localhost` だけ `http` が許される理由
> - **レイテンシの実測**（Neon シンガポールまで1往復 約77ms）と `PG_POOL_MAX` の話
>
> 逆に古いのは「サーバは各自の PC で動かす」という前提と、`localhost:4321` を
> エンドポイントにする手順。いまは Cloud Run の URL を使う。
...
Cloud Run へ移すのは、この 2 つが確認できてから（→ [deploy.md](deploy.md)）。
...
埋めるのは `DATABASE_URL` **だけ**でよい。他はすべてコメントアウトのままにする
（`PG_POOL_MAX` を下げると遅くなる。後述の「レイテンシ」参照）。
...
| 7 | 全員が同時に `npm run dev` | 接続エラーが出ない |

---

## レイテンシ（実測）

Neon はシンガポールなので **1往復 約77ms** かかる（ローカル docker は 0.5ms 未満）。
往復回数がそのまま体感に出るので、測った値を残しておく。

| 操作 | 所要時間 | 内訳 |
|---|---|---|
| 初回接続 | 約750ms | TLS ハンドシェイク + Neon のルーティング |
| `getState`（工場画面の10秒ポーリング） | **約390ms** | クエリ12本 |
| `getLeaderboard` | 約80ms | クエリ1本 |
| `getCollection` | 約75ms | クエリ1本 |

**ページを開いた直後だけ 1〜1.5 秒ほど何も表示されない**のはこれが理由で、異常ではない。
2回目以降はブラウザが接続を使い回すので早くなる。

### `PG_POOL_MAX` を下げてはいけない

`getState` は12本のクエリを並列に投げるため、接続数を絞ると往復が何波にも分かれる。

| `PG_POOL_MAX` | `getState` |
|---|---|
| 未設定（=10） | **約390ms** |
| 3 | 約1080ms（**約2.8倍**） |

共有 DB でも絞る必要は無い。**Neon 無料枠の実測は `max_connections=901`** なので、
10人が10接続ずつ持っても 100 にしかならない。下げるのは DB 側の上限に実際に
ぶつかったときだけ。

### さらに縮めたい場合

残る390msは「クエリ12本 ÷ 接続10本」の波によるもの。`getState` のクエリを削れば
縮むが、**Phase 2 で Cloud Run を東京に置くと往復自体が減る**ので、削る箇所の判断は
そこで測り直してからにしたほうがよい。
